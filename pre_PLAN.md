# Verification pre_PLAN — LED MUX Controller
...
**The agent generates files and makefile commands only — it does not run VCS compile or simulation.**

| Step | Who | Action |
|---|---|---|
| 1 | **Agent** | Create or update RTL/TB/UVM/SVA files for the current phase step |
| 2 | **Agent** | Print the **make commands** to run (see §Quick reference) |
| 3 | **Agent** | **Stop** — do not invoke `make`, `vcs`, or `simv` |
| 4 | **User** | Run commands on the course Linux VM; collect `*_comp.log` and `*_sim.log` |
| 5 | **User** | Prompt: *"check logfiles"* (or paste log paths / snippets) |
...
| 1.11 | Add test to **`test_lib.svh`** / filelist | `tb/test_lib.svh`, `tb/dut.f` |
...
| AC-P1-02 | No compile errors | `dut_comp.log` | No lines matching `Error-`, `Syntax error`, or `*E` (VCS error) |
...
LED_MUX_CONTROLLER_stu/
  tb/
    top_tb.sv              # clk, rst, DUT, interfaces, config_db, run_test()
    phase1_tb_top_test.sv  # Phase 1 gate test
    test_lib.svh           # `include phase1_tb_top_test
    dut.f                  # filelist includes top_tb + phase1 test
  sim/
...
**Agent delivers these commands; user executes them. Agent does not run them.**

```bash
# Setup (Linux / course VM)
cd LED_MUX_CONTROLLER_stu && source proj1.setup && cd sim

# Phase 1 — compile only
make compile TESTNAME=phase1_tb_top_test SEED=0

# Phase 1 — run (compile + sim)
make run TESTNAME=phase1_tb_top_test SEED=0

# Phase 1 — compile + run + gate script (user runs; agent checks logs after)
make phase1 TESTNAME=phase1_tb_top_test SEED=0
...
# Phase 3 — single P0 test
make run TESTNAME=led_decimal_42_test SEED=0

# Phase 3 — full P0 regression (end of Phase 3)
make regress_p0
make phase3

# After any run — user prompts agent to check:
#   sim/dut_comp.log
...
| pre_PLAN | ARCHITECTURE.md | TESTPLAN.md |
|---|---|---|
