...
module APB_Slave(i_prstn,i_pclk,i_paddr,i_pwrite,i_psel,i_penable,i_pwdata,o_prdata,o_pready,o_pslverr,o_led_enable,i_done);
...
parameter REG_NUM = 5;                                        //Dictates the size of the memory. Address span within a given slave equals 2**REG_NUM [iREG_NUM-1:0]. Default: 32 registers.
...
always @(posedge i_pclk or negedge i_prstn)
	if (!i_prstn) begin
	  ...
	  if ((i_pwrite)&&(o_pready)) begin
      	     mem[i_paddr[REG_NUM+WORD_LEN-1:WORD_LEN]]<=i_pwdata;
	     if (i_paddr =='h4000)
		o_led_enable <= i_pwdata[0];
	  end 
...
always @(posedge i_pclk or negedge i_prstn)
  if (!i_prstn) begin
    ...
    o_pready<=1'b1;
      ...
      o_pready<=1'b1;
      o_prdata<=mem[i_paddr[REG_NUM+WORD_LEN-1:WORD_LEN]];          
      if (i_paddr =='h4004)
	      o_prdata<=i_done;
      //Updating the o_prdata together with the rising edge of pready
    end
    else o_pready<=1'b0;
  end
